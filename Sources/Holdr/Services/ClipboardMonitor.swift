...
class ClipboardMonitor: ObservableObject {
    @Published var items: [HistoryItem] = [] {
        didSet {
            ...
            var newCategoryCounts: [String: Int] = [:]
            ...
            for item in items {
                ...
                // 3. Collect Category Counts
                newCategoryCounts[item.category.id, default: 0] += 1
                if let bid = item.appBundleID {
                    newCategoryCounts[HistoryItem.Category.app(bid).id, default: 0] += 1
                }
            }
    ...
    // Cache for O(1) category count lookup
