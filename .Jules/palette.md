...
## 2026-02-06 - SwiftUI Accessibility Ignore and Traits
**Learning:** Using `.accessibilityElement(children: .ignore)` on a SwiftUI view correctly hides its child elements from VoiceOver, but it also strips any implicit interactivity traits (like being a button) from the view.
**Action:** Always append `.accessibilityAddTraits(.isButton)` after `.accessibilityElement(children: .ignore)` when custom list items are meant to be interacted with as buttons.
