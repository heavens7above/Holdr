...
## 2026-02-05 - Semantic Badges in SwiftUI Lists
**Learning:** In macOS 12+ SwiftUI apps, replacing custom `HStack` list rows with `.badge(_ count: Int)` on `Label` components provides superior native styling, automatic zero-hiding, and semantic VoiceOver grouping without manual `.accessibilityElement(children: .combine)` wrappers.
**Action:** Always prefer `.badge()` over custom trailing `Text` views for item counts in navigation and sidebar lists.
