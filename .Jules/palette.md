...
## 2026-02-05 - Semantic Destructive Actions in macOS
**Learning:** Destructive actions in context menus (like Delete) should use the semantic `role: .destructive` parameter in SwiftUI. This ensures the text is rendered correctly (red) and matches standard macOS conventions, providing better visual cues and accessibility semantics to prevent accidental deletions.
**Action:** Always use `role: .destructive` on buttons that delete data or perform irreversible actions.
